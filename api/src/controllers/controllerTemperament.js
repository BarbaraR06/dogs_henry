...
const getByTemp = async (temperament) => {
    try {
        let resultApi = await axios.get(`https://api.thedogapi.com/v1/breeds?api_key=${API_KEY}`);
        let resultsTemp = resultApi.data.filter (t => t.temperament !== undefined);
        let resultsFilter = resultsTemp.filter (t => t.temperament.includes (`${temperament}`));
        ...
        const filterTempsDB = tempDB.filter (d => d.temperaments.includes(`${temperament}`))
